...
pub struct DelegatingImportTracer {
    delegates: Vec<OrdResolvedVc<Box<dyn ImportTracer>>>,
}

#[turbo_tasks::value_impl]
impl DelegatingImportTracer {
    #[turbo_tasks::function]
    pub fn new(delegates: Vec<OrdResolvedVc<Box<dyn ImportTracer>>>) -> Vc<Self> {
    ...
    T: CollectiblesSource + Copy + Send,
{
    fn peek_issues(self) -> CapturedIssues {
        let collectibles = {
            let mut c: Vec<OrdResolvedVc<Box<dyn ImportTracer>>> = self
                .peek_collectibles()
                .into_iter()
